    ...
    def pre_process(self) -> None:
        ...
        while True:
            if not os.path.isfile(f'./models/model_{i}.pkl'):
                ...
                while True:
                    bootstrapped_idxs = np.random.randint(self.log.count(), size=int(0.7*self.log.count()))
                    bootstrapped_actions = self.action[bootstrapped_idxs]
                    if len(set(bootstrapped_actions)) == 1500:
                        break

                self.model.fit(self.context[bootstrapped_idxs], self.action[bootstrapped_idxs])
                with open(f'./models/model_{i}.pkl', 'wb') as f:
                    pickle.dump(self.model, f)
                
                self.pscore = []
                for j in tqdm(range(self.n_rounds)):
                    self.pscore.append(self.model.predict_proba([self.context[j]])[0][self.action[j]])

                np.save(f'./models/pscores_{i}.npy', np.array(self.pscore))
