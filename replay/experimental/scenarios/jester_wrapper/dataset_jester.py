...
@dataclass
class JesterBanditDataset(BaseRealBanditDataset):
    ...
    def load_raw_data(self) -> None:
        ...
        log = preparator.transform(columns_mapping={'user_id': 'user_id',
                                                    'item_id': 'item_id',
                                                    'relevance': 'rating',
                                                    'timestamp': 'timestamp'}, data=self.logs)

        indexer = Indexer(user_col='user_id', item_col='item_id')
        indexer.fit(users=log.select('user_id'), items=log.select('item_id'))
        log = indexer.transform(df=log).toPandas()
        log['relevance'] = log['relevance'].apply(lambda x: int(x>=3.75))
        ...
        item_features_original = preparator.transform(columns_mapping={'item_id': 'item_id'}, data=self.items)
        self.item_features = indexer.transform(df=item_features_original)
        self.action_context = self.item_features.toPandas().drop(columns=['item_idx'],).to_numpy()

        #load user features and context
        user_features_original = preparator.transform(columns_mapping={'user_id': 'user_id'}, data=self.users)
        user_features = indexer.transform(df=user_features_original)
        user_features = user_features.toPandas()

        self.context = user_features.drop(columns=['user_idx'],).to_numpy()[log['user_idx'].to_numpy()]
        self.user_features = convert2spark(user_features)
    ...
    def pre_process(self) -> None:
        ...
        # self.pscore = np.ones(self.n_rounds)/self.n_rounds
